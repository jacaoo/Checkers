...
fun Board.play(from: Square, to: Square): Board =
    when (this) {
        is BoardWin -> error("Game is over")
            ...
            {"Invalid Play"}
            if (ThereIsAKill(this.player) || this.moves.values.toList()[from.index] is Queen ){
                var newBoard = this
                newBoard = newBoard.exchangekill(from, to)
                if (newBoard.ThereIsAKill(this.player)) {
...
fun Board.exchangekill(from: Square, to: Square): Board {
    ...
    if (piece != null && piece.canKill(to, from, this)) {
        if (piece is Pawn) {
            ...
            newMoves[from] = null
            return BoardRun(newMoves, player.other)
    ...
    when (this) {
        ...
        is BoardRun -> {
            if(this.FromSequenceMandatory != null) {
                require(from == this.FromSequenceMandatory ){"there is a mandatory sequence"}
            }
            val list = this.moves.values.toList()
